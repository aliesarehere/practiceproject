...
def fetch_bitcoin_data(start_date=None, end_date=None):
    if end_date is None:
        ...
        # If end_date is provided as a string, convert it to a datetime object
        end_date = datetime.strptime(end_date, '%Y-%m-%d')

    if start_date is None:
        # Set start_date as 1825 days before the end_date
        start_date = (end_date - timedelta(days=1825)).strftime('%Y-%m-%d')
    else:
        # If start_date is provided as a string, convert it to a datetime object
        start_date = datetime.strptime(start_date, '%Y-%m-%d').strftime('%Y-%m-%d')

    # Convert end_date back to string for yfinance input
...
def add_technical_indicators(data):
    ...
    # MACD Histogram (difference between MACD and MACD signal)
...
# Set a threshold for dropping columns with too many NaN values (e.g., 10% missing data)
threshold = 0.1 * len(bitcoin_data)  # 10% threshold
columns_to_drop = bitcoin_data.columns[bitcoin_data.isna().sum() > threshold]

# Drop columns with too many NaN values
if len(columns_to_drop) > 0:
    print(f"Indicators causing too many NaNs (removed): {columns_to_drop.tolist()}")
    bitcoin_data.drop(columns=columns_to_drop, axis=1, inplace=True)
...
# If any NaNs still remain, fill remaining NaNs with a small constant (e.g., 0)
bitcoin_data.fillna(0, inplace=True)  # Fill any remaining NaNs
...
class LSTMModel(nn.Module):
    def __init__(self, input_size=8, hidden_layer_size=100, dropout=0.3, output_size=1, num_layers=2):
...
def objective(trial):
    hidden_layer_size = trial.suggest_int('hidden_layer_size', 50, 300)
    ...
    for epoch in range(EPOCHS):
        ...
        if epoch % 10 == 0:
            print(f'Objective Trial - Epoch {epoch}: Train Loss = {running_loss / len(train_loader):.4f}, Val Loss = {val_loss / len(val_loader):.4f}')
...
model.load_state_dict(torch.load('best_model.pth', weights_only=True))  # Updated to weights_only
...
formatter = mdates.DateFormatter('%b %d, %Y')  # This will display dates as 'Jan 01, 2024', for example
